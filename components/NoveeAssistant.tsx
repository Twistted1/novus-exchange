...
export default function NoveeAssistant() {
  ...
  function speakText(text: string) {
    ...
    const bestVoice =
      preferredVoices.find(v => v.name.includes('Google US English')) ||
      preferredVoices.find(v => v.name.includes('Google UK English Female')) ||
      preferredVoices.find(v => v.name.includes('Microsoft Zira')) || // Good Windows voice
      preferredVoices.find(v => v.name.includes('Samantha')) || // Good Mac voice
        ...
        {isOpen && (
          ...
          >
            ...
            <form onSubmit={handleSend} className="p-3 bg-gray-900/80 border-t border-white/10 flex gap-2">
              ...
              />
              <button type="button" aria-label="Start voice input" onClick={startListening} className={`p-2 rounded-full ${isListening ? 'text-red-500 animate-pulse' : 'text-gray-400 hover:text-teal-400'}`}>
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" /></svg>
              </button>
              <button type="submit" aria-label="Send message" className="p-2 bg-teal-600 text-white rounded-full hover:bg-teal-500 transition-colors">
